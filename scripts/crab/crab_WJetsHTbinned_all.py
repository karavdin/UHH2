...
inputDatasets = ['/WJetsToLNu_HT-*To*_TuneCP5_13TeV-madgraphMLM-pythia8/RunIIAutumn18MiniAOD-102X_upgrade2018_realistic_v15-v*/MINIAODSIM']
inputDatasets = autocomplete_Datasets(inputDatasets)
requestNames = []
for x in inputDatasets:
    name = x.split('/')[1]
    modified_name =name.replace('_TuneCP5_13TeV-madgraphMLM-pythia8','')
    if 'ext1' in x:
        modified_name += '_ext1'
    elif 'ext2' in x:
        modified_name += '_ext2'
...
try:
    config.Data.outLFNDirBase = '/store/user/%s/RunII_102X_v1_ProductionMarch19/' % (getUsernameFromSiteDB())
